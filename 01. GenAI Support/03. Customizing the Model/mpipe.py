...
# global variable for LLM flow control
is_processing = False
...
# Helfer-Funktion zum Extrahieren der Punkte
def get_coords(indices):
    return [{"x": round(face_landmarks.landmark[i].x, 3), 
             "y": round(face_landmarks.landmark[i].y, 3)} for i in indices]
             
def ask_ollama_worker(landmark_json):
    """
    Worker-Thread: Sendet Daten an Ollama und setzt die Flagge 
    nach Erhalt der Antwort wieder zurück.
    """
    global is_processing
    prompt = (
        "Interpret these facial landmarks (normalized 0-1) and describe "
        "the emotion or head pose in one short sentence: " + landmark_json
    )
    payload = {
        "model": "custom-qwen",
        "prompt": prompt,
        "stream": False,
        "keep_alive": "1h"  # Hält das Modell für eine Stunde im Speicher
    }
    
    try:
        response = requests.post(OLLAMA_API_URL, json=payload, timeout=None) # Langer Timeout für langsame Hardware
        result = response.json().get("response")
        print(f"\n[Ollama Prediction]: {result}")
    except Exception as e:
        print(f"\n[Ollama Error]: {e}")
    finally:
        is_processing = False # Erlaubt die nächste Anfrage
    ...
    while cap.isOpened():
        ...
        if results.multi_face_landmarks:
            for face_landmarks in results.multi_face_landmarks:
                ...
                # 1. Nur die absolut notwendigen Kennzahlen für Ollama berechnen
                relevant_data = {
                     "oval_width": round(abs(face_landmarks.landmark[454].x - face_landmarks.landmark[234].x), 3),
                     "mouth_open": round(abs(face_landmarks.landmark[13].y - face_landmarks.landmark[14].y), 3),
                     "left_eye": round(abs(face_landmarks.landmark[159].y - face_landmarks.landmark[145].y), 3)
                }
        
                # 2. Den Thread nur starten, wenn das Modell bereit ist
                if not is_processing:
                   is_processing = True
                   data_str = json.dumps(relevant_data)
                   thread = threading.Thread(target=ask_ollama_worker, args=(data_str,))
                   thread.daemon = True
                   thread.start()
